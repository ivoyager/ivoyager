...
var section_headers := ["", "Orbital Characteristics"]
...
func _get_label_value(section: int, data_index: int) -> Array:
	...
	match typeof(value):
		...
		TYPE_STRING:
			...
			if enable_wiki and data_size > 8 and line_data[8]:
				value_wiki = value # may be used as wiki link
	if !value_txt:
	...
	if value_wiki and _wiki_titles.has(value_wiki): # value is wiki link
		_meta_lookup[value_txt] = value_wiki
