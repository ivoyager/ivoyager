...
**New!** Repository [ivbinary_maker](https://github.com/ivoyager/ivbinary_maker) has tools for converting asteroid raw source data into ivoyager_assets/asteroid_binaries, and Saturn rings image data into a 1d texture for rings.shader.


### Added
* Added 'rings.shader' that makes Saturn rings from 1d data, and dynamically responds to phase angle!
* Added spacecrafts. For now, just ISS and Hubble. (Juno has been added as an object, but it uses a fallback sphere model because the open-source Juno model crashes in GLES2 operation.)
* Added 'visited' asteroids (with 3D models): Vesta, Bennu, Eros, Itokawa and Arrokoth.
* Asteroid orbit lines can be shown by group, or all at once. Yeah, all ~78,000 of them!
* Mouse-over text now identifies almost anything on the world screen, including bodies, orbit lines and points (yes, including the ~78,000 asteroids). For the shader trickery involved in this feature, see [IVFragmentIdentifier](https://github.com/ivoyager/ivoyager/blob/master/tree_nodes/fragment_identifier.gd). 
* New View system allows saving 'Views' that optionally keep selection, camera orientation, HUDs visibility & color, and/or time state.
...
* Many new GUI widgets to interact with the new features and content.


### Changed
* Updated asteroid binary data to reflect Feb 2023 source data. We filtered by magnitude to include ~78,000 asteroids in ivoyager_assets/asteroid_binaries, further reduced in the Planetarium web app to ~48,000. The full set that can be generated by ivbinary_maker without magnitude cutoff is ~1.14 million. (All sets include both numbered and multiopposition asteroids.)
...
* Improved many GUI widgets (appearance & program modularity).
...
* Somewhat less inacurate Trojan orbits. Ocillation of longitude and semi-major axis are now phased correctly. Keep in mind that these libration orbits will always be rough approximations (there are no closed-form equations to code). It's not bad however and will keep getting better.
