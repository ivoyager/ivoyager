...
func _ready() -> void:
	...
	IVGlobal.connect("user_pause_changed", Callable(self, "_update_buttons"))
	_timekeeper.connect("speed_changed", Callable(self, "_update_buttons"))
	_minus.connect("pressed", Callable(self, "_increment_speed").bind(-1))
	_plus.connect("pressed", Callable(self, "_increment_speed").bind(1))
	if !IVGlobal.disable_pause:
		_pause.connect("pressed", Callable(self, "_change_paused"))
	else:
		_pause.queue_free()
		_pause = null
	if IVGlobal.allow_time_reversal:
		_reverse.connect("pressed", Callable(self, "_change_reversed"))
	else:
		_reverse.queue_free()
		_reverse = null
	_update_buttons()
